...
var consonantsLessY = ["b", "c", "d", "f", "g", "h", "j", "k", "l", "m", "n", "p", "q", "r", "s", "t", "v", "w", "x", "z", "B", "C", "D", "F", "G", "H", "J", "K", "L", "M", "N", "O", "P", "Q", "R", "S", "T", "V", "W", "X", "Z"];
...
var wordTranslator = function(input) {
  for (i = 0; i < input.length; i++) {
    ...
    } else if (consonants.includes(input[0]) && consonants.includes(input[1]) && consonantsLessY.includes(input[2])) {
      translation.push(input.slice(3) + input[0] + input[1] + input[2] + "ay");
    } else if ((consonants.includes(input[0]) && consonantsLessY.includes(input[1])) || (quArray.includes(input[0] + input[1]))) {
